    ...
    <div className="min-h-screen">
      <div className="flex min-h-screen">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className={`border-b px-4 py-4 shadow-sm backdrop-blur lg:hidden ${isDark ? "border-slate-700 bg-slate-900/80" : "border-slate-200 bg-white/80"}`}>
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className={`flex h-9 w-9 items-center justify-center rounded-xl text-sm font-bold ${isDark ? "bg-sky-500/20 text-sky-300" : "bg-sky-100 text-sky-700"}`}>
                  M
                </div>
                <div>
                  <h1 className={`text-base font-bold ${isDark ? "text-slate-100" : "text-slate-900"}`}>Mi Pensum</h1>
          ...
          <div className="mx-auto flex w-full max-w-[1500px] flex-1 items-start gap-8 px-4 py-6 md:px-8 lg:py-8">
            <main id="pensum-section" className="w-full max-w-4xl scroll-mt-6">
              ...
              <section className={`mb-6 rounded-2xl border p-6 shadow-sm backdrop-blur ${basePanel}`}>
                <p className={`text-xs font-semibold uppercase tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                  Resumen
                </p>
                <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
                  <h2 className={`text-2xl font-black tracking-tight md:text-3xl ${isDark ? "text-slate-100" : "text-slate-900"}`}>
                    Pensum academico
                  </h2>
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${isDark ? "bg-sky-500/20 text-sky-200" : "bg-sky-100 text-sky-700"}`}>
              ...
              {isLoading && (
                <p className={`rounded-2xl border p-4 shadow-sm ${isDark ? "border-slate-700 bg-slate-900 text-slate-200" : "border-slate-200 bg-white text-slate-700"}`}>
                  Cargando pensum...
                </p>
              )}
              {!isLoading && error && (
                <p className={`rounded-2xl border p-4 shadow-sm ${isDark ? "border-red-900 bg-red-950/50 text-red-200" : "border-red-200 bg-red-50 text-red-700"}`}>
                  {error}
                </p>
              )}
            ...
            <div id="progress-desktop" className="sticky top-6 hidden scroll-mt-6 flex-col gap-4 lg:flex">
              <PensumSelectorCard
                className="w-80"
                value={selectedPensum}
                options={PENSUM_OPTIONS}
